...
# Specify input
# =============================================================================
source_dir = '/home/etsai/BNL/Research/GIWAXS_tomo_2019C2/RLi2_TOMO_data_201902/RLi2/waxs/raw/'
out_dir = '../results/'
...
peak_list = [
        # center, size, peak
        [[575, 479-227], [60, 10], 'sum002'],
        # 11L
        [[525, 548], [180, 10], 'sum11L'],
        #[[525, 223-shift], [180, 10], 'sum11Lb'],
        # 02L
        [[608, 607], [20, 10], 'sum02L'],
        #[[603, 172-shift], [30, 10], 'sum02Lb'],
        # 12L
        [[589, 684], [58, 6], 'sum12L'], 
        #[[589, 110-shift], [58, 6], 'sum12Lb'],
        # 20L
        [[300, 744], [40, 15], 'sum20L'],
        #[[323, 56-shift], [30, 15], 'sum20Lb'],
        # 21L
        [[250, 782], [40, 15], 'sum21L'],
        #[[280, 26-shift], [40, 15], 'sum21Lb'],
        # 22
        [[588, 858], [40, 15], 'sum22L'],
        # Si
        [[385, 631], [12, 12], 'sumSi'],
        #[[400, 151-shift], [12, 12], 'sumSib'],
        # background
...
if flag_load_raw_data:
    ...
    for ii, infile in enumerate(infiles):
        if ii%fraction==0: # Quick checck peak positions
    ...
    plt.colorbar()
    plt.title('Average over {} data (fraction={}) \n {}'.format(N_files,fraction,infiles[0]))
...
if flag_get_peaks:   
    ...
    if flag_load_parellel:
        ...
        for ii, infile in enumerate(infiles):
            #if ii%10==0:
            if ii%100 ==0: print('========{}========'.format(ii))
            temp = get_peaks(infile, peak_list, verbose=0, flag_LinearSubBKG=flag_LinearSubBKG)
            results.append(temp)
...
list_peaks = ['sum002',
 ...
 'sumBKG0']
list_peaks = []
data_sort, sino_dict = get_sino_from_data(df_peaks, list_peaks=list_peaks, flag_rm_expbg=1, flag_thr=0) #flag_thr=2 for binary
...
if 1:
    ...
    plt.xlabel('deg')
    peaks_idx = label_peaks(theta, sum_sino)
    plt.title(peak)
    ...
    x = {} 
    for jj, angle in enumerate(theta[peaks_idx]):
        x[jj] = pd.DataFrame([[angle, 'sum20L']], columns=['angle','peak']); 
...
domain_angle_offset = theta[peaks_idx]
