...
analysis_PATH='/home/etsai/BNL/Research/GIWAXS_tomo_2019C3/RLi/waxs/GI_tomo/'
analysis_PATH in sys.path or sys.path.append(analysis_PATH)
import fun_peaks, fun_tomo_recon
...
list_peaks = []
flag_rm_expbg = 1
flag_thr = 1 #flag_thr=2 for binary
data_sort, sino_dict = get_sino_from_data(df_peaks, list_peaks=list_peaks, flag_rm_expbg=flag_rm_expbg, flag_thr=flag_thr) 
...
## Plot sino
title_st = '{}\nflag_rm_expbg={}, flag_thr={}'.format(filename, flag_rm_expbg, flag_thr)
plot_sino(sino_dict, fignum=31, title_st=title_st, vlog10=[0, 5.5])

if flag_save_png:
    fn_out = out_dir+filename+'peaks_sino_deg' 
    fn_out = check_file_exist(fn_out)
    plt.savefig(fn_out, format='png')
        
## Do and plot recon
if flag_tomo:
    recon_all = get_plot_recon(sino_dict, rot_center=30, algorithms = ['gridrec', 'fbp', 'tv'], fignum=40)
    if flag_save_png:
...
# =============================================================================
# Label peak positons (deg) of a sino & store to a df
# =============================================================================
...
#peak =  'sum20L'
    ...
    ## Get the angles (where peaks are) for one most dominant domain
...
for ii, offset in enumerate(domain_angle_offset):  
    ...
    ## Plot sino
    title_st = '{}\nflag_normal{},width{}'.format(filename, flag_normal,width) if ii==0 else ''
...
for ii in domains_use:      
    ...
    if ii==0:
        ...
        centers = [center]
    recon_binary = do_segmentation(recon, centers, width=2, fignum=0) #
...
thr_select = [8, 8, 9, 9, 18, 5, 5, 4, 4, 4]
for ii, recon in enumerate(recon_all_list):
    #temp = recon.reshape(recon.size)
    #thr = np.mean( temp[np.argsort(temp)[-100:]] )*0.5
    #thr = np.median(recon)*2
    thr = thr_select[ii]
    print(thr)
    ...
    recon_adjust = recon_binary/np.max(recon)
    #recon_adjust[recon_adjust<np.mean(recon_adjust)*2] = 0
    plt.subplot(1,Ndomain,ii+1)  
    plt.imshow(recon_adjust); plt.axis('off')
...
# Generate a guess 
# =============================================================================
domains_use = [0, 1, 3, 4, 6, 7, 8] #np.arange(0, len(recon_all_list)) #[0, 1, 3, 4, 6]  
recon_all_list_normal = []
for ii in domains_use:      
    recon_adjust = recon_all_list[ii].copy()
    recon_adjust = recon_adjust/np.max(recon_adjust)
    thr = 0.3
    recon_adjust[recon_adjust<thr] = 0
    recon_all_list_normal.append(recon_adjust)
mask = (recon!=0).astype(float)
mask_nan = mask.copy()
mask_nan[mask==0] = np.nan
mask_nan[np.max(recon_all_list_normal,0)==0] = np.nan
