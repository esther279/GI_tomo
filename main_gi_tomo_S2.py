...
filename_peak = './GI_tomo/param/S2_peaks.txt'
...
if 2 in run_steps:     
    ...
    peaks.get_peaks(infiles[0], peak_list, phi_max=360, verbose=2, FS=16)
...
if 4 in run_steps: 
    ...
    data_sort, sino_dict = tomo.get_sino_from_data(df_peaks, list_peaks=list_peaks, flag_rm_expbg = 1, thr=0.1, binary=None) 
    ...
    plt.imshow(sino_dict['sino_allpeaks'][:,:,0], extent=[0, 50, 360, 0], cmap='gray_r')
    plt.axis('auto')
    if flag_save_png:
    ...
    ## Do and plot recon
    rot_center = 29
    recon_00L = tomo.get_plot_recon(sino_dict['sino_allpeaks'][:,:,0], theta=sino_dict['theta'], rot_center=rot_center, algorithms = ['gridrec', 'fbp', 'tv'], fignum=13+1)
...
if 5 in run_steps: 
    ...
    #peak =  'sum20L'
        ...
        sum_sino_all.append(sum_sino/np.max(sum_sino))
        if flag_log10: 
        ...
        plt.text(-15, np.max(sum_sino)*0.7, peak[3:], fontsize=6, color=color)
        if ii==0: plt.title(HOME_PATH+', '+filename)
        
        peaks_idx = tomo.label_peaks(theta, sum_sino, onedomain=1, fontsize=6)
    ...
    plt.imshow(sum_sino_all, extent=[0, 360, 1, len(sum_sino_all)], cmap='terrain'); 
    plt.axis('auto')
    plt.xlabel('phi (deg)')
    plt.colorbar()
...
if 6 in run_steps: 
    ...
    domain_angle_offset = theta[sum_sino>2e4]
...
if 7 in run_steps: 
    ...
    for ii, offset in enumerate(domain_angle_offset):  
        ...
        if 1:
            temp_recon = seg.do_seg_sino(sino_dm, thetas = sino_dict['theta'], rot_center=25, algo='fbp')  
            recon_all_list.append(temp_recon)
        else:
            temp = tomo.get_plot_recon(sino_dm, theta = sino_dict['theta'], rot_center=29-4, algorithms = [algo], title_st=title_st, fignum=None, colorbar=True)
...
# =============================================================================
if 1:
    x = np.asarray(Image.open("./mask_S2.png").convert("L").rotate(0).resize((50,50)))
...
if 8 in run_steps:
    ...
    if 1:   ## Tune recon param
        ...
        #---- Recon with thr
        recon_thr = seg.do_seg_sino(sino_dm, thetas = sino_dict['theta'], thr_range=[0.5, 1, 0.02], rot_center=25, algo='fbp')  
    ...
    plt.figure(38, figsize=[4,8]); plt.clf() 
    plt.subplot(1,3,1)     
...
# =============================================================================    
if 11 in run_steps:    
    domain_angles = [124, 94, 100, 112] #[18, 128, 160] ## in deg
    print("domain_angles = {}".format(domain_angles))
    idx_use = []
    ...
    plt.figure(50, figsize=[20,12]); plt.clf()
    for ii in idx_use:      
        recon = recon_all_list[ii]
        ...
        if 1: ## Threshold
            ...
            recon_plot = recon_plot/np.max(recon_plot)*mask
        ...
        ## Plot
        ax = plt.subplot2grid((4, 7), (channel, 0), colspan=2); 
    ...
    ax = plt.subplot2grid((4, 7), (0, 2), rowspan=3, colspan=4); ax.cla()
    ax.set_facecolor('k')    
